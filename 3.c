...
int exec_proc_per_two_command(char *A[], char *B[]/*, char **in_out_A, char **in_out_B*/, char *in_out[])
{
//	char *A[] = **list[0];
//	char *B[] = **list[1];
	int pipefd[2];
	pipe(pipefd);
	int fd[2];
	fd[0] = redir_in(/*in_out_A[0]*/in_out[0]); //for A
	fd[1] = redir_out(/*in_out_B[1]*/in_out[1]); //for B
	if(fork() == 0) {
		dup2(fd[0], 0);
		dup2(pipefd[1], 1);
		close(pipefd[0]);
...
//	char ***mas_in_out = NULL;
	...
	while(!exit_proc(all_commands)) {
		...
		exec_proc_per_two_command(all_commands[0], all_commands[1]/*, mas_in_out[0], mas_in_out[1]*/, in_out);
